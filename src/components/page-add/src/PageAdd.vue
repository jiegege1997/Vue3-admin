<template>
  <div class="add-dialog">
    <el-dialog v-model="visible" title="Tips" width="30%" @close="handleClose">
      <hy-form :form-items="formItems" v-model="modelValue" :span="24">
      </hy-form>
      <slot></slot>
      <template #footer>
...
const handleConfirm = () => {
  ...
  if (props.modelValue.id) {
    if (props.pageName === "role") {
      systemStore.updatePageData(
        props.pageName,
        { ...props.modelValue, ...props.otherValue },
        props.modelValue.id
      );
    } else {
